...
"""
Testint EhBasic with Mike Naberezny's py65 project to emulate a 6502 CPU in Python for easy testing.
Mike's original repository: https://github.com/mnaberez/py65
I did some light modifications for my purpose, also for simplicy this is a single Python file now,
combined the needed source from py65, and also some crazy code from me to implement a rudimentary
6502 emulator with basic I/O functionaility for EhBASIC.
"""
...
class CPU6502(object):
    ...
    def step(self):
        ...
        # LGB's modification: return with the instruction code
        # this can be used to "trap" with unimplemented opcode for example
        return instructCode
...
class Tester(object):
	...
	def curses_uninit(self):
		curses.nocbreak()
		self.stdscr.keypad(0)
		curses.echo()
		curses.endwin()
		print("NCURSES: uninit")

	def curses_getkey(self, wait):
		self.stdscr.nodelay(not wait)
		c = self.stdscr.getch()
		if c == 10:
			c = 13
		return (0 if c < 0 or c > 0x7F else c)

	def curses_printchar(self, char):
		if char == 13:
			return
		try:
	...
	def curses_printstring(self, string):
		try:
	...
	def curses_refresh_if_any(self):
		if self.need_refresh:
			self.need_refresh = False
			self.stdscr.refresh()

	def TestProgram(self, program, loadaddr, startaddr):
		self.mem = bytearray(loadaddr) + program + bytearray(0x10000 - loadaddr - len(program))
		if len(self.mem) != 0x10000:
			return "Internal Error: memory is not 64K long"
		self.cpu = self.cpu_class(memory = self.mem, pc = startaddr)
		banner = "6502 Tester: program loaded to ${:04X}-${:04X}, starting at ${:04X}".format(loadaddr, loadaddr + len(program) - 1, startaddr)
		print(banner)
		self.curses_init()
		try:
			self.curses_printstring(banner + "\nPress CTRL-C to exit.\n\n")
			while True:
				opcode = self.cpu.step()
				if self.cpu.unimplemented:
					if opcode == 0x02:
						self.curses_printchar(self.cpu.a & 0x7F)
					elif opcode == 0x12:
						self.cpu.a = self.curses_getkey(wait = False)
					elif opcode == 0x22:
						self.cpu.a = self.curses_getkey(wait = True)
					elif opcode == 0x32:
						s = "User app guided exit at PC ${:04X}: ".format(self.cpu.old_pc)
						while True:
							self.cpu.old_pc += 1
							c = self.mem[self.cpu.old_pc]
							if c == 0:
								break
							s += chr(c)
						self.curses_uninit()
						...
						raise RuntimeError("Unimplemented opcode: ${:02X} at PC ${:04X}".format(opcode, self.cpu.old_pc))
				if self.cpu.processorCycles > 40000:
					self.curses_refresh_if_any()
...
def boot(argv):
	if len(argv) != 4:
		return "Usage: " + argv[0] + " loadfile.bin load-addr start-addr"
	try:
		loadfile, loadaddr, startaddr = argv[1], int(argv[2], base = 0), int(argv[3], base = 0)
		if loadaddr < 0:
			raise ValueError("Load address cannot be negative")
		if startaddr < loadaddr:
			raise ValueError("Start address cannot be less than load address")
		with open(loadfile, "rb") as program:
			program = bytearray(program.read())
		if loadaddr + len(program) > 0x10000:
			raise ValueError("Program is too big to fit in memory from given load address")
		if startaddr >= loadaddr + len(program):
			raise ValueError("Start address cannot be after the end of loaded program")
	except IOError as e:
		return "Error loading program: " + str(e)
	except ValueError as e:
		return "Parameter error: " + str(e)
	return Tester(cpu_class = CPU6502).TestProgram(program = program, loadaddr = loadaddr, startaddr = startaddr)
